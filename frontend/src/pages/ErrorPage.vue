<template>
  <div class="container-wrapper">
    <div class="icon-sad-face container">
      <h3 class="error__title">{{$t(`message[${errors.title}]`)}}</h3>
      <div class="error__description" v-if="$t(`message[${errors.description}]`)" v-html="$t(`message[${errors.description}]`)"></div>

      <div>
        <router-link class="button" v-if="isRequiredLogin" :to="{name: 'Login'}">{{ $t('message["로그인 다시 하기"]') }}</router-link>
      </div>
    </div>
  </div>
</template>
